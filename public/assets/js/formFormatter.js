...
function changeSelectedProduct(val) {
	for (var i = 0; i < productPrices.length; i++) {
		if (productPrices[i].product_id == val) {
			curPrice = productPrices[i].selling_price;
		}
	}
	changePrice();
}
  ...
  for (var i = 0; i < groupedUnpaid.length; i++) {
    if (groupedUnpaid[i].customerID == $('#unpaidCustomer').val()) {
      ...
      for (var x = 0; x < groupedUnpaid[i].orderDetails.length; x++) {
        row = '<tr><td style="font-weight: bold;"><input type="text" readonly class="regular-input" value="'+groupedUnpaid[i].orderDetails[x].deliveryReceipt+'" name="paymentDR">'+'</td><td>'+groupedUnpaid[i].orderDetails[x].productName+'</td><td class="">'+groupedUnpaid[i].orderDetails[x].qty+' bags</td><td class="text-right">Php '+groupedUnpaid[i].orderDetails[x].totalAmt+'</td><td class="text-center"><input type="checkbox" value="' + groupedUnpaid[i].orderDetails[x].totalAmt +'" id="'+ x +'" class="checkbox_payment" onclick="checkbox_add()"></td></tr>';
