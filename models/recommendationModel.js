...
exports.getRecommendation2 = function(next) {
	var sql = "select pt.product_name, case when max(t.completed_qty) is null then 0 else max(t.completed_qty) end as completed_orders, case when max(t.pending_qty) is null then 0 else max(t.pending_qty) end as pending_orders, pt.qty as current_stock, case when (max(t.completed_qty) is null and max(t.pending_qty) is null) then 0 else ceiling((max(ifnull(t.completed_qty, 0)) + max(ifnull(t.pending_qty, 0)))/7) end as moving_avg, case when (max(t.completed_qty)+max(t.pending_qty)) is null then max(t.limiter) - pt.qty else ceiling((max(t.completed_qty) + max(t.pending_qty))/7) + max(t.limiter) - pt.qty end as recommended, max(t.limiter) as safety_limit from (select sh.product_id, sum(sh.qty) as completed_qty, null as pending_qty, null as limiter from sales_history as sh where datediff(curdate(), sh.scheduled_date) <= 8 and datediff(curdate(), sh.scheduled_date) >= 1 and sh.order_status = 'Completed' union select sh.product_id, null, sum(sh.qty) as ordered_qty, null from sales_history as sh where datediff(curdate(), sh.scheduled_date) <= 8 and datediff(curdate(), sh.scheduled_date) >= 1 and sh.order_status != 'Completed' union select pt.product_id, null, null, pt.safety_limit from product_table as pt ) as t join product_table as pt using(product_id) group by t.product_id";
